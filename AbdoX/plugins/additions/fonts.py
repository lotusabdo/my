...
async def zahrafa(c: Client, m: Message):
    ...
    if len(text_list) < 2:
        await m.reply_text("↢ للزخرفه عربي ارسل\nمثال : زخرفه سوفيا √", reply_to_message_id=m.id)
        return
    text = text_list[1].split(None, 1)[1] if len(text_list[1].split()) > 1 else text_list[1]
    if len(text) > 20:
        await m.reply_text("↢ فقط ارسل كلمه واحدة للزخرفة √", reply_to_message_id=m.id)
        return
    ...
    # هنا يتم تنفيذ زخرفة النص
        ...
        if re.match("\n", str(m.text)):
            await m.reply_text("↢ لا استطيع تزيين النص اذا كان اكثر من سطر واحد √", reply_to_message_id=m.id)
            return
    EmojeS = [
        ' 𓁻',
        ' 𓏴  ',
